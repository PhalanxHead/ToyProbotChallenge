...
namespace ToyRobotChallenge
{
    ...
    public class CommandParser
    {
        ...
        public CommandParser(bool isUsingCaseSensitivity = true)
        {
        ...
        {
            // Split the "Arguments" up by their allowed delimiters.
            // Note that the first element will have the "PLACE " still on the front,
            // and garbage will stay on the end of the last element.
            var splitCommandUnit = commandUnit.Split(validArgument_ArgumentDelimiters, StringSplitOptions.RemoveEmptyEntries);
            ...
            {
                var splitSegment = argumentSegment
                        .Split(validCommand_ArgumentDelimiters, StringSplitOptions.RemoveEmptyEntries)
                        .Select(x => x.Trim());

                arguments.AddRange(splitSegment.Where(x => !x.Equals(PLACE_COMMAND_PREFIX, _stringComparisonMethod)));
            }
